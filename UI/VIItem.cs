    ...
    internal class VIItem : VirtualItem
    {
        ...
        public override void Paint(Canvas g, VirtualPanelArgs e)
        {
            if (Hover)
            {
                ...
                g.String(Key, e.Panel.Font, Style.Db.PrimaryColor, rect_text, FormatFlags.Center | FormatFlags.VerticalCenter);
            }
            else
            {
                if (bmp == null) bmp = Value.SvgToBmp(icon_size, icon_size, Style.Db.Text);
                g.Image(bmp, rect_icon);
                g.String(Key, e.Panel.Font, Style.Db.Text, rect_text, FormatFlags.Center | FormatFlags.VerticalCenter);
